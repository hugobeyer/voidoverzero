/**
 * OtMatic UVs App Module - Main initialization and content rendering
 * OtMatic UVs Documentation v1.0.0
...
const App = {
    ...
    renderContent() {
        ...
        if (!content) {
            console.error('OtMatic: Content element not found!');
            return;
        }

        if (!DOCS_DATA || !DOCS_DATA.panels) {
            console.error('OtMatic: DOCS_DATA not loaded!');
            return;
        }

        console.log('OtMatic: Rendering', DOCS_DATA.panels.length, 'panels');
        ...
        console.log('OtMatic: Content rendered');
    },
    ...
    initScrollAnimations() {
        ...
        // Observe panels, groups, and param cards
        document.querySelectorAll('.panel-section, .param-card, .group-section').forEach(el => {
            observer.observe(el);
        });

        // Make panels visible immediately (they're above the fold)
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('OtMatic: DOM loaded');
    console.log('OtMatic: DOCS_DATA', DOCS_DATA);
    console.log('OtMatic: Panels count', DOCS_DATA?.panels?.length);
    App.init();
});
