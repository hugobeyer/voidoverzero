...
const App = {
    ...
    createParamCard(panelId, groupId, param, index) {
        ...
        // Check for admin content override
        const adminItem = window.adminItemMap ? window.adminItemMap[`param-${key}`] : null;
        const displayName = adminItem ? adminItem.name : param.name;
        const displayDesc = adminItem ? adminItem.description : param.desc;
        const richContent = adminItem ? adminItem.content : null;

        // Use rich content if available, otherwise fallback to description
        const contentHtml = richContent ?
            `<div class="param-card-rich-content">${richContent}</div>` :
            `<p class="param-card-desc">${displayDesc}</p>`;
